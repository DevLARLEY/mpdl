...
from data.pywidevine.L3.cdm.formats import wv_proto4_pb2 as wv_proto4
from data.pywidevine.L3.cdm.session import Session
from data.pywidevine.L3.cdm.key import Key
...
class Cdm:
    ...
    def _parse_init_data(self, init_data_b64):
        parsed_init_data = wv_proto4.WidevineCencHeader()
        try:
    ...
    def set_service_certificate(self, session_id, cert_b64):
        ...
        message = wv_proto4.SignedMessage()
        ...
        service_certificate = wv_proto4.SignedDeviceCertificate()
    ...
    def get_license_request(self, session_id):
        ...
        if self.raw_pssh:
            license_request = wv_proto4.SignedLicenseRequestRaw()
        else:
            license_request = wv_proto4.SignedLicenseRequest()
        ...
        if not self.raw_pssh:
            license_request.Type = wv_proto4.SignedLicenseRequest.MessageType.Value('LICENSE_REQUEST')
            license_request.Msg.ContentId.CencId.Pssh.CopyFrom(session.init_data)
        else:
            license_request.Type = wv_proto4.SignedLicenseRequestRaw.MessageType.Value('LICENSE_REQUEST')
            license_request.Msg.ContentId.CencId.Pssh = session.init_data  # bytes

        if session.offline:
            license_type = wv_proto4.LicenseType.Value('OFFLINE')
        else:
            license_type = wv_proto4.LicenseType.Value('DEFAULT')
        license_request.Msg.ContentId.CencId.LicenseType = license_type
        license_request.Msg.ContentId.CencId.RequestId = session_id
        license_request.Msg.Type = wv_proto4.LicenseRequest.RequestType.Value('NEW')
        license_request.Msg.RequestTime = int(time.time())
        license_request.Msg.ProtocolVersion = wv_proto4.ProtocolVersion.Value('CURRENT')
        ...
        if session.privacy_mode:
            if session.device_config.vmp:
                ...
                vmp_hashes = wv_proto4.FileHashes()
            ...
            encrypted_client_id_proto = wv_proto4.EncryptedClientIdentification()
    ...
    def provide_license(self, session_id, license_b64) -> bool:
        ...
        lic = wv_proto4.SignedLicense()
        try:
            lic.ParseFromString(base64.b64decode(license_b64))
        except DecodeError:
        ...
        for line in text_format.MessageToString(lic).splitlines():
            self.logger.debug(line)
        ...
        session.session_key = oaep_cipher.decrypt(lic.SessionKey)
        ...
        lic_hmac.update(lic.Msg.SerializeToString())

        self.logger.debug(
            "calculated sig: {} actual sig: {}".format(lic_hmac.hexdigest(), binascii.hexlify(lic.Signature)))

        if lic_hmac.digest() != lic.Signature:
            ...
            with open("parsed_lic.bin", "wb") as f:
                f.write(lic.SerializeToString())
            self.logger.info("continuing anyway")

        self.logger.debug("key count: {}".format(len(lic.Msg.Key)))
        for key in lic.Msg.Key:
            if key.Id:
                key_id = key.Id
            else:
                key_id = wv_proto4.License.KeyContainer.KeyType.Name(key.Type).encode('utf-8')
            encrypted_key = key.Key
            iv = key.Iv
            type = wv_proto4.License.KeyContainer.KeyType.Name(key.Type)
