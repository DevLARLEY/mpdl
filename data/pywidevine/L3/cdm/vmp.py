try:
    from google.protobuf.internal.decoder import \
        _DecodeVarint as _di  # this was tested to work with protobuf 3, but it's an internal API (any varint decoder might work)
except ImportError:
...
class WideVineSignatureReader(FromFileMixin):
    ...
    def __init__(self, buf):
        ...
        if (len(extra) != 1 or (extra[0] > 1)):
            raise Exception(
                "Unexpected 'ismainexe' field value (not '\\x00' or '\\x01'), please check: {0}".format(extra))
